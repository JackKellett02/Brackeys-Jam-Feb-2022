...
public class TrampolineControllerScript : MonoBehaviour
{
    ...
    private void LaunchCollision(Collider2D collision)
    {
        ...
        dotProduct = Vector2.Dot(colliderVelocity, launchNormal);
        if(dotProduct == 0.0f)
        {
            int a = 0;
        }
    ...
    private void AnimateTrampoline()
    {
        ...
        //Calculate Points
        for (int i = 1; i < pointsVector2.Length - 1; i++)
        { //Interpolate between the start and end point for the current item in the array.
