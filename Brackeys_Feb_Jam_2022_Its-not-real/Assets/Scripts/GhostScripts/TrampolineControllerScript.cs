...
public class TrampolineControllerScript : MonoBehaviour {
	...
	[SerializeField]
	[Range(0.0f, 10.0f)]
	private float stretchMultiplier = 1.0f;

	[SerializeField]
	[Range(1.0f, 20.0f)]
	private float trampolineLifetime = 5.0f;

	[SerializeField]
	private Color trampolineColour = Color.blue;
	...
	private float stretchLength = 0.0f;
	...
	void Awake() {
		...
		//Update trampoline colour.
		trampolineLineRenderer.material.color = trampolineColour;
	}
	...
	void Update() {
		if (trampolineAnimating) {
			...
			centerPointTransform.position = originalCenterPoint;
	...
	private void LaunchCollision(Collider2D collision) {
		...
		stretchLength = ((pointsVector2[pointsVector2.Length - 1] - pointsVector2[0]).magnitude) * stretchMultiplier;
		...
		//TEST
		colliderVelocity = GetDirectionFromPlayerToMidpoint(collision);
		...
		if (dotProduct > 0.0f) {
			launchNormal = Vector2.zero - launchNormal;
		}
	...
	private Vector2 GetDirectionFromPlayerToMidpoint(Collider2D collider2D) {
		Vector2 centerPos = new Vector2(centerPointTransform.position.x, centerPointTransform.position.y);
		Vector2 collider2DPos = new Vector2(collider2D.transform.position.x, collider2D.transform.position.y);

		return (centerPos - collider2DPos);
	}
	#endregion
	...
	public void SetTrampolinePoints(Vector2 a_StartPoint, Vector2 a_EndPoint, int numberOfPoints) {
		...
		originalCenterPoint = centerPointTransform.position;
		...
		//Start the lifespan countdown to deactivate the trampoline.
		StartCoroutine("TrampolineLifeSpan");
	}
	#endregion
}
